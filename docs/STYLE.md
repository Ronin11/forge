...
## 13. Conflict-resistant conventions (M9)

Registries (modes, executors, tools, parsers, plugins, checks) are generated by
`go generate` from directory listings — never hand-edited; regeneration is the
merge resolution. Schema changes are ULID-named migration files (never edited
after landing). Changelog entries are fragment files under `changes/`. One concept
per file; new tests go in new files where reasonable. `gofmt`/`goimports` are
gates on Forge's own repo and part of the default checks proposed for other
repositories.
