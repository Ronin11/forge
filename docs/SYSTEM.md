...
**src/envelope.rs** defines the agent's structured-result JSON schema and parser (`Envelope`, `NeedsInput`, `Change`, `Claim`) that `verify` and `supervisor` use to interpret an agent's summary, checks, claims, and requests for input. `Change` belongs to the stored envelope: the verifier derives `changes[]` from git for every provider, while the model-facing schema omits it.
...
**web/ (forge-web binary)** is a browser client at the same seam as the TUI: every read is a forge verb's JSON and the live feed is `events --follow` piped through as server-sent events; write routes include answer, withdraw, retry, land, initiative settings, gc, and deploy, each through its corresponding CLI verb (`forge answer`, `forge withdraw`, `forge retry`, `forge land`, `forge initiative set`, `forge gc`, and `forge deploy`) via `forge-client`. `forge task set` is CLI-only in this checkout. Workflow and plugin controls also use CLI verbs; `POST /hooks/<project>/<name>` hands webhook delivery to `forge job fire` (docs/CLIENT.md, "Webhooks"). Every request carries a token generated into `FORGE_HOME/web.token`, except a hook, whose own per-hook bearer token the kernel checks; the server binds loopback unless told otherwise. `web/src/app.js` renders the `/tasks`, `/tasks/<id>`, and `/tasks/<id>/run` views client-side.
...
Landing and every push run from the same kernel-owned repository. `integrate`
fetches the verified branch from the clone (the one command that reads the
agent's metadata, and it does not run hooks), clones a landing tree from the
kernel repository, and does the merge and the re-verification there; the
task branch and the base are then pushed from the kernel repository by object
id, and the tree verified is checked to be the commit pushed. The tests step's
`verify/<id>`, `publish`, and `forge gc` go through the same fetch-then-push
(`git::push_ref`), so a pre-push hook, credential helper or `core.sshCommand`
in an agent's clone never runs. A conflicting base reaches the clone by a
hardened fetch (`place_branch`), never a push. `src/git.rs` holds the list of
every `Git::new` call site and a test fails when one is added.
