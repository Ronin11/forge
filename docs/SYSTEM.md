...
**src/sandbox.rs** builds the `bwrap` command line that isolates agent and check subprocesses behind a read-only host filesystem and a tmpfs `$HOME`, with only the task's clone, the agent binary, and configured toolchain/cache paths bound in. On by default; refuses to run without `bwrap` unless `FORGE_SANDBOX=0`.
