...
**Stage 4. Kernel seams out of engine.rs and cli.rs (hand). Done 2026-09-14: journal.rs, prompts.rs, queue.rs (with a task_queued event), landing.rs, operation.rs, OpRow and a Timer for the op recorder's twelve arguments, Rewind carrying the new base instead of mutating the task; engine.rs 2583 → 1540 lines.** In order:
