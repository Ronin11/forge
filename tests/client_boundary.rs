//! The real boundary test. `tests/boundary.rs` is protected — `forge.toml`
//! marks it "the one rule a task may not relax" — so it stays a plain
//! substring scan of each manifest's `[dependencies]` text and cannot grow
//! past that. This file is where the boundary is actually enforced: it
//! parses every member manifest with the `toml` crate (so a real dependency
//! table, not a text search, decides what's forbidden — `forge-client`
//! never collides with `forge` the way a substring check risks), covers
//! `dev-dependencies` and `build-dependencies` too, tests `repomap`
//! separately as a tool rather than a client, and checks that no client
//! source file invokes a `forge` verb `docs/CLIENT.md` doesn't document.
