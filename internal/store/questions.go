...
func (tx *Tx) CreateQuestion(ctx context.Context, a *Attempt, q protocol.QuestionRequest) (*Question, error) {
	...
	out := &Question{ID: model.NewID(), AttemptID: a.ID, TargetID: a.TargetID, WorkID: t.WorkID, Text: q.Text, Options: q.Options, Context: q.Context, Checkpoint: q.Checkpoint, AskedAt: tx.now}
	if _, err := tx.Exec(ctx, `INSERT INTO questions (id, attempt_id, target_id, work_id, text, options, context, checkpoint, asked_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		out.ID, out.AttemptID, out.TargetID, out.WorkID, out.Text, jsonOrNull(out.Options), jsonRaw(out.Context), nullString(out.Checkpoint), formatTime(tx.now)); err != nil {
		return nil, fmt.Errorf("insert question: %w", err)
	}
	if err := tx.Journal(ctx, "question.asked", EntityQuestion, out.ID, map[string]any{"attempt_id": a.ID, "target_id": a.TargetID, "checkpoint": q.Checkpoint}); err != nil {
		return nil, err
	}
	return out, nil
}

// AnswerQuestion records the answer and re-queues the Target (pinned to its
...
func (tx *Tx) AnswerQuestion(ctx context.Context, questionID, answer, by string) (*Question, error) {
	...
	q.Answer, q.AnsweredBy, q.AnsweredAt = answer, by, tx.now
	if _, err := tx.Exec(ctx, `UPDATE questions SET answer = ?, answered_by = ?, answered_at = ? WHERE id = ?`, answer, by, formatTime(tx.now), questionID); err != nil {
		return nil, fmt.Errorf("answer question %s: %w", questionID, err)
	}
	if err := tx.Journal(ctx, "question.answered", EntityQuestion, questionID, map[string]any{"by": by, "target_id": q.TargetID}); err != nil {
		return nil, err
	}
	var open int
...
const questionColumns = `id, attempt_id, target_id, work_id, text, options, context, checkpoint, answer, answered_by, asked_at, answered_at`
...
func scanQuestions(iter func(func(*sql.Rows) error) error) ([]Question, error) {
	...
	err := iter(func(rows *sql.Rows) error {
		var q Question
		var options, context, checkpoint, answer, by, answeredAt sql.NullString
		var asked string
		if err := rows.Scan(&q.ID, &q.AttemptID, &q.TargetID, &q.WorkID, &q.Text, &options, &context, &checkpoint, &answer, &by, &asked, &answeredAt); err != nil {
			return fmt.Errorf("scan question: %w", err)
		}
		q.Checkpoint, q.Answer, q.AnsweredBy = checkpoint.String, answer.String, by.String
		if context.Valid {
