...
//!
//! - L0: is the worktree consistent? Clean tree, at least one commit, and
//!   `forge.toml` untouched, so the branch under test cannot change what it
//!   is verified against.
//! - L1: do the repository's declared checks pass, read from the trusted
//!   base commit and run by Forge in the sandbox?
//! - L2: do the task's own acceptance commands pass? These are the
//!   operator's definition of done, declared with `--check` when the task
//!   was created.
//!
...
use crate::checks::{CheckResult, run_one};
...
pub async fn verify(s: Subject<'_>, agent: &Outcome) -> Result<Verdict> {
    ...
    if agent_reason.is_none() {
        ...
        let l0_ok = v.checks.iter().all(|c| c.ok);
    ...
    let (state, reason) = decide(agent_reason.as_deref(), &v.checks);
    v.state = state;
    v.reason = reason;
    Ok(v)
}
...
pub fn decide(agent_failure: Option<&str>, checks: &[CheckResult]) -> (AttemptState, String) {
    ...
    match v.state {
        ...
        _ => {
            ...
            for c in v.checks.iter().filter(|c| !c.ok) {
                ...
                for l in c.tail.lines() {
                    fb.push_str(&format!("    {l}\n"));
                }
            }
            fb.push_str("Fix the failures, leave the tree clean, and commit.");
            fb
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn verdict_table() {
        let cases: Vec<(Option<&str>, Vec<CheckResult>, AttemptState, &str)> = vec![
            (
            ...
            (None, vec![], AttemptState::Unverified, "no L1 or L2"),
            (
                None,
        ...
        for (agent, checks, want, reason) in cases {
            let (got, why) = decide(agent, &checks);
            assert_eq!(got, want, "agent={agent:?} checks={checks:?}");
            assert!(
                why.contains(reason),
